...
- **Referrals**: Management, commission tracking, conversion pipeline, Lead Generator (API_BASE_LEADS integration with dynamic filters, 1000-record limit, and WhatsApp bulk messaging via WHU API with full audit logging in `gerador_leads_whatsapp_logs` table). Lead Generator WhatsApp Dispatch Platform with async queue processing, rate limiting (2/s, 30/min), 30-day recurrence blocking, daily duplicate prevention, retry (max 3x), real-time polling progress, operational dashboard with metrics/charts/logs, and RBAC-based dispatch permission control. **Indicações Meu Painel** (`IndicacoesMeuPainel.jsx`): agent-specific dashboard showing referral sales from `API_VENDAS_INDICACAO_AGENTES`, correlated via `erp_agent_id`. Backend route `GET /functions/indicacoes-agent-dashboard` filters by logged-in agent's ERP ID, applies `valores_pagos=SIM` rule. Displays KPI cards (vendas, valor total, ticket médio), evolution charts (vendas/dia, faturamento/dia via Recharts), and recent sales table. Period filters via `DashboardFilters` component.
...
- **ReferralPipeline Data Loading**: Query uses `enabled: !!user && !isLoadingAgents` to prevent race conditions. Agent resolution is done inside `queryFn` with fallback lookup. All agent ID comparisons use `String()` casting. QueryKey uses `'referrals-pipeline'` prefix to avoid cache collisions.
...
- **ERP Agent ID Mapping**: `erp_agent_id` BIGINT nullable column on `agents` table with unique partial index. Service layer (`backend/src/services/erpIntegrationService.js`) provides `getAgentByErpId()`, `getErpAgentMap()`, `findAgentByNormalizedName()`, `resolveAgentFromErp()`. Frontend `Agents.jsx` includes ERP ID field in agent form (create/edit). `convertKeysToSnake` handles `erpAgentId` → `erp_agent_id` automatically.
...
- **WhatsApp Dispatch Platform (Lead Generator)**: Professional dispatch system with async queue processing (`backend/src/services/whatsappQueueService.js`). Tables: `gerador_leads_queue` (queue items with batch tracking), `gerador_leads_audit_log` (unauthorized access attempts), `gerador_leads_rate_config` (configurable rate limits), `gerador_leads_conversoes` (conversion tracking with normalized phone matching). Features: rate limiting (2/s, 30/min default), 30-day recurrence blocking per number+template, daily duplicate prevention, intelligent retry (max 3x), real-time polling via `/lead-generator-queue-status/:batchId` (2s interval). Frontend: `LeadGenerator.jsx` with tabs (Gerador | Painel de Disparos), `LeadGeneratorDashboard.jsx` Cockpit Comercial with: executive cards (total/success/failures/blocks/success-rate/ROI-per-dispatch), commercial cards (conversions/conversion-rate/total-sales-value/unique-leads), visual commercial funnel (leads→sent→success→conversions with stage rates), line charts (dispatches by day, value generated by day), bar charts (conversions by day, performance by user, performance by team, dispatches by hour, products sold), user ranking table (dispatches/conversions/rate/value sorted by value), campaign analysis table (by batch_id with leads/sent/conversions/rate/value, paginated), recent dispatches table with pagination, recent conversions table, intelligent alerts (conversion <1%, failure >10%, high blocks, dispatch volume drop). Collapsible sections for organized layout. Global date range filter with presets (Today/7d/30d/90d). Backend returns `byDay`, `byBatch`, `byProduct` from `getDashboardMetrics` and `getConversionMetrics`. RBAC: `DISPATCH_FORBIDDEN_TYPES = ['vendas', 'sales', 'bom_auto_atendente', 'support', 'collection', 'pre_sales', 'post_sales']` — these agent types cannot dispatch, retry, or view dashboard. Unauthorized attempts logged in audit_log.
- **Lead Generator Metrics Audit**: Automated daily audit system for Lead Generator metrics. Core logic in `executeMetricsAudit()` function (reusable). Routes: `GET /functions/lead-generator-metrics-audit` (on-demand audit report), `GET /functions/run-lead-generator-audit` (execute and persist to DB). Checks: vendas_sem_disparo, disparos_sem_venda, possiveis_duplicidades, metric recalculation (taxa/ROI). Results stored in `gerador_leads_auditoria` table. Cron job via `node-cron` runs daily at 03:00 (`runLeadGeneratorAudit()`), defaults to last 24h. Exported from `functions.js`, imported in `server.js`. Accepts filters: `from`, `to`, `userId`, `teamId`.
...
- **ERP Bom Pastor**: Integrated for CPF lookup in the Referral system. API `API_CPF_INDICADOR` uses `cpf=` parameter (formatted with dots/dash, e.g. `cpf=123.456.789-00`). The `documento=` parameter does NOT filter — it returns all records. Response is deduplicated server-side (Map by `contrato_servicos`) and only first raw record is returned to frontend to avoid sending thousands of records (~6.5MB → ~5KB).
- **ERP Bom Auto API**: Integrated for vehicle/client consultation in the Bom Auto module (API_TESTE_BOM_AUTO endpoint).
- **ERP API_DADOS_VENDAS_INDICACOES**: Sales data API for Lead Generator ROI metrics and Commission Management validation. Returns real contracts with `cel_indicador`, `cpf_indicador`, `nome_indicador`, `contrato_servicos`, `valor_contrato`, `data_contrato`, `valores_pagos`, etc. Used by `GET /functions/lead-generator-roi-metrics` to cross-reference dispatches with actual ERP sales, and by `GET /functions/referral-paid-sales` to validate commission eligibility (only `valores_pagos=SIM` sales generate commissions). Commission matching priority: CPF > phone > name. Phone normalization strips `55` DDI prefix from `lead_number` to match `cel_indicador`. Currency parsing handles BR format (`1.234,56`). Conversion rule: `normalized_phone == cel_indicador AND data_contrato >= first_dispatch_date`. Conversion rate uses unique leads (not event count) as denominator.