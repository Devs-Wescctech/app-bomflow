...
- **ERP API_DADOS_VENDAS_INDICACOES**: Sales data API for Lead Generator ROI metrics. Returns real contracts with `cel_indicador`, `contrato_servicos`, `valor_contrato`, `data_contrato`, etc. Used by `GET /functions/lead-generator-roi-metrics` to cross-reference dispatches with actual ERP sales. Phone normalization strips `55` DDI prefix from `lead_number` to match `cel_indicador`. Currency parsing handles BR format (`1.234,56`). Conversion rule: `normalized_phone == cel_indicador AND data_contrato >= first_dispatch_date`. Conversion rate uses unique leads (not event count) as denominator.