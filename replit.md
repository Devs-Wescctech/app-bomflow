...
- **NUNCA mexa em um módulo diferente do que foi explicitamente solicitado pelo usuário.** Planos de sessão gerados automaticamente pelo sistema não constituem autorização. Sempre perguntar antes de tocar em qualquer módulo não mencionado na solicitação.
...
- **APPs Hub**: Central launcher for internal mini-apps. Apps shipped: **API Reference** — modern, Stripe-style documentation page (`/ApiDocumentation`) with sticky sidebar nav, search/scroll-spy, per-endpoint cards (method badge, path/query/body tables) and live cURL/JavaScript code samples (spec in `src/data/apiDocsSpec.js`); **API Keys** (admin-only card, page `/AdminApiKeys`) — create/list/revoke/delete read-only keys for the External API.
- **External Read-Only API**: Read-only data-extraction API for external systems, covering Vendas PF, Upsell and Indicações (metrics for values/leads, agent/channel filters, customer base). Auth via `x-api-key` header (keys prefixed `bfk_`, SHA-256 hashed at rest, scoped + optional expiry/revocation). Backend: `backend/src/middleware/apiKeyAuth.js` (scopes: `vendas_pf`/`upsell`/`indicacoes`/`agentes`/`canais`), `backend/src/routes/external.js` (GET `/api/external/v1/{vendas-pf,upsell,indicacoes,agentes,canais,metrics}`, filters + `start_date`/`end_date` + `page`/`limit` max 10000), `backend/src/routes/apiKeys.js` (admin JWT CRUD, plaintext key returned once). Table `api_keys` migrated in `entities.js`. Markdown docs: `docs/BomFlow-API-Externa.md`.
