...
curl_setopt_array($curl, array(
  CURLOPT_URL => "http://erp.wescctech.com.br:8080/BP_MULTI/api/API_BOM_PET_SAUDE_3PETS_DEPENDENTES?documento=$cpf&pedido=$pedido",
...
if ($response<>''){
	...
	foreach ($response as $chave => $valor){
		...
		foreach ($valor as $chave_um => $valor_um){
			...
			if ($chave_um=='nome'){$nome_pet = $valor_um;}		
			if ($chave_um=='cel'){$cel = $valor_um;}
			if ($chave_um=='canal'){$canal = $valor_um;}
			if ($chave_um=='data_nasc'){$data_nasc_pet = $valor_um;}
			if ($chave_um=='sexo'){$sexo_pet = $valor_um;}
		...
		";
		*/
		$pesquisa = array($cor1, $cor2, $raca, $nome_pet, $data_nasc_pet, $sexo_pet);
		if (in_array($pesquisa, $dependentes)==false){
			//echo "nome: $nome_pet $data_nasc_pet<br>";
			$dependentes[] = array($cor1, $cor2, $raca, $nome_pet, $data_nasc_pet, $sexo_pet);
			$cont++;
		}
	}
	$total_pets=$cont;
