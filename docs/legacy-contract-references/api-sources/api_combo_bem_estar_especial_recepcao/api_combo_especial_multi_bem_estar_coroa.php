...
curl_setopt_array($curl, array(
  CURLOPT_URL => "http://erp.wescctech.com.br:8080/BOMPASTOR/api/API_COMBO_ESPECIAL_MULTI_COROA?documento=$cpf&pedido=$pedido",
...
if (is_array($response) && !empty($response)) {
	foreach($response as $dados){
		...
		$quantidade_coroa==$dados["quantidade_total"];
	}
	$tem_coroa='SIM';
}else{
	$tem_coroa='NAO';
