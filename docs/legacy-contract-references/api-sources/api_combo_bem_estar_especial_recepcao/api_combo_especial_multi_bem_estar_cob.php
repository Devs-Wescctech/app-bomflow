...
curl_setopt_array($curl, array(
  CURLOPT_URL => "http://erp.wescctech.com.br:8080/BOMPASTOR/api/API_COMBO_ESPECIAL_MULTI_BEM_ESTAR_COB?documento=$cpf&pedido=$pedido",
...
if ($response<>''){
	...
	foreach ($response as $chave => $valor){
		...
		foreach ($valor as $chave_um => $valor_um){
			...
			if ($chave_um=='descricao'){$descricao = $valor_um;}
		}
		...
		*/
			if ($total_valor>2){
				if($descricao<>'NOVO COMBO MULTI BEM ESTAR'){$tot_dependentes_cob=$tot_dependentes_cob+$total_valor;
				}else{
