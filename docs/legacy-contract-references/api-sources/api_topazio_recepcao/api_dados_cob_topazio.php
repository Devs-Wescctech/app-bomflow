...
curl_setopt_array($curl, array(
  CURLOPT_URL => "http://erp.wescctech.com.br:8080/BP_MULTI/api/API_DADOS_COB_TOPAZIO?documento=$cpf&pedido=$pedido",
...
if ($response<>''){
	...
	foreach ($response as $chave => $valor){
		...
		foreach ($valor as $chave_um => $valor_um){
			...
			if ($chave_um=='observacoes'){$observacoes = $valor_um;}
			if ($chave_um=='descricao'){$descricao_plano = $valor_um;}
		}
		...
		*/
			...
			if ($descricao_plano=='POÇOS DE CALDAS - TOPAZIO'){
				$plano='POÇOS DE CALDAS';	
			}
		$cont++;
	}
