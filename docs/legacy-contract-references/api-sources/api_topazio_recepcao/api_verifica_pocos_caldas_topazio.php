...
curl_setopt_array($curl, array(
  CURLOPT_URL => "http://erp.wescctech.com.br:8080/BP_MULTI/api/API_PESQUISA_VERIFICA_TOPAZIO_POCOS?documento=$cpf&pedido=$pedido",
...
if (is_array($response) && !empty($response)) {
	foreach($response as $dados){
		$pedido_pocos =$dados["pedido"];
		$documento_pocos=$dados["documento"];
		$total_valor_pocos=$dados["total_valor"];
		$descricao_pocos=$dados["descricao"];
	}
}
?>