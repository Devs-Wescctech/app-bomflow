...
curl_setopt_array($curl, array(
  CURLOPT_URL => "http://erp.wescctech.com.br:8080/BP_MULTI/api/API_COMBO_MULTI_SELECAO_TANATO?documento=$cpf&pedido=$pedido",
...
if (is_array($response) && !empty($response)) {
	foreach($response as $dados){
		$pedido_tanato =$dados["pedido"];
		$documento_tanato=$dados["pedido"];
		$total_valor_tanato=$dados["total_valor"];		
	}
	$tem_tanato='SIM';
}else{
	$tem_tanato='NAO';
	$total_valor_tanato=0;
}
?>