...
**NÃO INSISTIR em writes de DB.** 3 writes falharam (função, canal, menu).

**CLEANUP CONCLUÍDO (2026-06-08):** todos os reverts aplicados. menu_id→NULL ok; pessoas_contratos removido ok; funcoes_usuarios do acesso.api restaurado para baseline limpo de conta de API = `{198859503 USUARIO_SISTEMA, 251861329 API_ETHERUM}` (removidas as 2 funções de vendedor 2094530 CANAL DE VENDA + 2094535 VENDEDOR que tinham sido adicionadas no teste). Os 13 pedidos 'M' de teste foram HARD DELETADOS com segurança (verificado: zero linhas filhas em todas as ~30 FKs de `pedidos`; DELETE transacional, 13 apagados, 0 restantes). Códigos de situação de `pedidos`: A=aprovado, C=cancelado, M=montagem/rascunho, I, P, R. Scripts scratch em backend/scripts/ removidos.
