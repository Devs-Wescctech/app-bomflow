    ...
    endpoints: [
      {
        id: "bp-consulta",
        method: "GET",
        path: "/bom-pet/consulta",
        title: "Consulta Cliente",
        description:
          "Consulta titular, situação financeira e pets do plano no ERP. Aceita CPF (11 dígitos) ou nome completo (convertido para maiúsculas). Retorna 409 se o nome for ambíguo (mais de um titular).",
        auth: true,
        query: [
          { name: "documento", type: "string", required: false, description: "CPF do titular (11 dígitos)" },
          { name: "nome", type: "string", required: false, description: "Nome completo do titular (enviado em maiúsculas)" },
        ],
        response: {
          status: 200,
          example: {
            titular: { contratante: "MARIA SILVA", documento: "123.456.789-00", celular: "(31) 99999-9999", contrato_id: 123, situacao_financeira: "Adimplente" },
            inadimplente: false,
      ...
      },
      {
        id: "bp-parcelas",
        method: "GET",
        path: "/bom-pet/parcelas/:documento",
        title: "Parcelas Pendentes",
        description:
          "Lista as parcelas/boletos pendentes do cliente no ERP (títulos em aberto). Inclui link de pagamento quando disponível na fonte.",
        auth: true,
        params: [{ name: "documento", type: "string", required: true, description: "CPF do titular" }],
        response: { status: 200, example: { parcelas: [{ sequencia: 3, valor: 59.9, data_vencimento: "2026-06-10", url_pagamento: null }] } },
      },
      {
        id: "bp-utilizacoes",
        method: "GET",
        path: "/bom-pet/utilizacoes/:documento",
        title: "Utilizações por Documento",
        description: "Lista os atendimentos Bom Pet registrados para um cliente.",
        auth: true,
        params: [{ name: "documento", type: "string", required: true, description: "CPF do titular" }],
        response: { status: 200, example: [{ protocolo: "BP2607300001", pet_nome: "THEO", status_atendimento: "Pendente" }] },
      },
      {
        id: "bp-atendimentos-create",
        method: "POST",
        ...
        body: [
          ...
          { name: "pet_contrato_id", type: "number", required: true, description: "ID do contrato de serviço no ERP; não identifica sozinho o pet" },
          { name: "pet_nome", type: "string", required: true, description: "Nome/descrição do pet" },
          { name: "remocao_local", type: "string", required: true, description: "Local da remoção" },
          { name: "remocao_endereco", type: "string", required: true, description: "Endereço da remoção" },
          { name: "clinica_nome", type: "string", required: false, description: "Clínica veterinária" },
          { name: "parceiro_nome", type: "string", required: false, description: "Parceiro operacional / direcionamento" },
          { name: "telefone_contato", type: "string", required: true, description: "Telefone de contato" },
          { name: "observacoes", type: "string", required: false, description: "Observações" },
      ...
      {
        id: "bp-atendimentos-list",
        method: "GET",
        path: "/bom-pet/atendimentos",
        title: "Listar Atendimentos",
        description: "Lista atendimentos com filtros opcionais (sem campos de veículo).",
        auth: true,
        query: [
          ...
          { name: "atendente", type: "string", required: false, description: "E-mail do atendente" },
          { name: "data_inicio", type: "string", required: false, description: "Data inicial (YYYY-MM-DD)" },
      ...
      },
      {
        ...
        method: "PUT",
        path: "/bom-pet/atendimentos/:id",
        title: "Tratar Atendimento",
        description:
          "Atualiza o status (whitelist Pendente/Solucionado/Cancelado). Solucionado exige ao menos 1 imagem anexada. marcar_pet_falecido=true exige data_falecimento, registra localmente e cria ou atualiza com segurança a característica DATA_FALECIMENTO (id 55435402) da Pessoa individual do pet quando a integração estiver habilitada.",
        auth: true,
        params: [{ name: "id", type: "string", required: true, description: "ID do atendimento" }],
        body: [
          { name: "status_atendimento", type: "string", required: true, description: "Pendente | Solucionado | Cancelado" },
          { name: "observacoes_tratamento", type: "string", required: false, description: "Observação do tratamento" },
          { name: "marcar_pet_falecido", type: "boolean", required: false, description: "Marca o pet como Falecido no Bom Flow e solicita sincronização com o ERP" },
          { name: "data_falecimento", type: "string", required: false, description: "Obrigatória quando marcar_pet_falecido=true; formato YYYY-MM-DD" },
        ],
        response: { status: 200, example: { id: 1, status_atendimento: "Solucionado", erp_falecimento_sync_status: "confirmed" } },
      },
      {
        id: "bp-atendimentos-sync-falecimento",
        method: "POST",
        path: "/bom-pet/atendimentos/:id/sincronizar-falecimento",
        title: "Reenviar Falecimento ao ERP",
        description:
          "Reenvia com segurança uma marcação local pendente. A operação cria a característica 55435402 quando ausente ou atualiza seu valor quando vazio, é idempotente para a mesma Pessoa e data e nunca sobrescreve uma data divergente.",
        auth: true,
        params: [{ name: "id", type: "string", required: true, description: "ID do atendimento" }],
        response: { status: 200, example: { id: 1, erp_falecimento_sync_status: "confirmed" } },
      },
      {
        id: "bp-imagens",
        method: "POST",
        path: "/bom-pet/atendimentos/:id/imagens",
        title: "Upload de Comprovantes",
        description: "Anexa imagens ao atendimento (multipart/form-data, campo 'imagens', até 10 arquivos de 5MB). Verifica existência e escopo do atendimento antes de gravar.",
        auth: true,
        params: [{ name: "id", type: "string", required: true, description: "ID do atendimento" }],
        response: { status: 201, example: { imagens: [{ id: 1, url: "/api/bom-pet/imagens/abc.jpg" }] } },
      },
      {
        id: "bp-imagem-download",
        method: "GET",
        path: "/bom-pet/imagens/:filename",
        title: "Download de Comprovante",
        description: "Serve a imagem do comprovante de remoção com autenticação e escopo por registro (atendente só acessa imagens dos próprios atendimentos).",
        auth: true,
      ...
      },
      {
        id: "bp-termo",
        method: "PATCH",
        path: "/bom-pet/atendimentos/:id/termo",
        title: "Dados da Autorização",
        description: "Salva os dados do termo/autorização de cremação usados no PDF e no texto copiado para o parceiro.",
        auth: true,
        params: [{ name: "id", type: "string", required: true, description: "ID do atendimento" }],
      ...
      },
      {
        id: "bp-historico",
        method: "GET",
        path: "/bom-pet/atendimentos/:id/historico",
        title: "Histórico do Atendimento",
        description: "Retorna a trilha de alterações do atendimento (status, usuário, observação).",
        auth: true,
        params: [{ name: "id", type: "string", required: true, description: "ID do atendimento" }],
        response: { status: 200, example: [{ status_anterior: "Pendente", status_novo: "Solucionado", usuario: "a@b.com" }] },
      },
      {
        id: "bp-contadores",
        method: "GET",
        path: "/bom-pet/atendimentos/contadores",
        title: "Contadores",
        description: "KPIs do painel: pendentes, solucionados, cancelados e total.",
        auth: true,
        response: { status: 200, example: { pendentes: 2, solucionados: 10, cancelados: 1, total: 13 } },
      },
    ],
  },
  {
