...
export function calculateCommissionValue(level, isCorretor = false) {
  if (isCorretor) {
    switch (level) {
      case 3: return 400;
      case 2: return 300;
...
export function getLevelDescription(level, isCorretor = false) {
  if (isCorretor) {
    switch (level) {
      case 3: return { badge: "🏆 Corretor Nível 3", color: "bg-purple-100 text-purple-800" };
      case 2: return { badge: "⭐ Corretor Nível 2", color: "bg-amber-100 text-amber-800" };
      default: return { badge: "Corretor Nível 1", color: "bg-blue-100 text-blue-800" };
    }
  }
  switch (level) {
...
export function getNextLevelInfo(totalConversions, currentLevel, isCorretor = false) {
  if (isCorretor) {
    if (currentLevel >= 3) {
      return "Nível máximo atingido! Comissão por venda: R$ 400,00";
    }
    if (currentLevel === 2) {
      const remaining = 13 - totalConversions;
      return `Faltam ${remaining} indicação${remaining !== 1 ? 'ões' : ''} para Nível 3 (R$ 400,00)`;
    }
    const remaining = 4 - totalConversions;
    return `Faltam ${remaining} indicação${remaining !== 1 ? 'ões' : ''} para Nível 2 (R$ 300,00)`;
  }
  if (currentLevel >= 3) {
...
export const COMMISSION_RULES_CORRETOR = [
  { level: 1, min: 1, max: 3, value: 200, description: "1-3 indicações (Corretor)" },
  { level: 2, min: 4, max: 12, value: 300, description: "4-12 indicações (Corretor)" },
  { level: 3, min: 13, max: null, value: 400, description: "13+ indicações (Corretor)" },
];
