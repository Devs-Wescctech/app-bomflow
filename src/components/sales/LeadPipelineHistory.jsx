...
import { Check, Clock, ArrowRight, CircleDot, Trophy, XCircle, Timer, DollarSign } from "lucide-react";
import { format } from "date-fns";
import { ptBR } from "date-fns/locale";

const STAGES = [
  { id: 'novo', label: 'Novo', color: 'gray', gradient: 'from-gray-400 to-gray-500' },
  { id: 'abordado', label: 'Abordado', color: 'blue', gradient: 'from-blue-400 to-blue-600' },
  { id: 'qualificado', label: 'Qualificado', color: 'purple', gradient: 'from-purple-400 to-purple-600' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        {/* Progress Fill */}
        <div 
          className="absolute top-6 left-0 h-1 bg-gradient-to-r from-blue-500 via-purple-500 to-emerald-500 rounded-full transition-all duration-500"
          style={{ width: `${Math.max(0, (currentIndex / (STAGES.length - 2)) * 100)}%` }}
        />
        
        {/* Stage Dots */}
        <div className="relative flex justify-between">
          {STAGES.filter(s => s.id !== 'fechado_perdido').map((stage, index) => {
            const visit = stageVisits[stage.id];
            const isCompleted = !!visit && currentStage !== stage.id;
            const isCurrent = currentStage === stage.id;
            const isPending = !visit && currentStage !== stage.id;
              ...
              >
                ...
                <button
                  ...
                  className={`
                    ...
                    ${isCurrent 
                      ? `bg-gradient-to-br ${stage.gradient} border-white dark:border-gray-900 shadow-lg shadow-${stage.color}-500/40 scale-110 ring-4 ring-${stage.color}-200 dark:ring-${stage.color}-900` 
                      : isCompleted 
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4">
        ...
        <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-3 border border-gray-100 dark:border-gray-700/50">
          ...
          <p className="text-lg font-bold text-gray-900 dark:text-white">
            {Object.keys(stageVisits).length} / {STAGES.length - 1}
          </p>
        </div>
