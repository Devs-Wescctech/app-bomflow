...
export default function LeadImportReferrals() {
  ...
  const { data: agentTypesData = [] } = useQuery({
    queryKey: ['agent-types-referral-import'],
    queryFn: async () => {
      try {
        return await base44.entities.AgentType.list();
      } catch {
        return [];
      }
    },
  ...
  const isAllowed = user?.role === 'admin' || agentType === 'admin' ||
    ...
    (agentType || '').endsWith('_supervisor') || agentType === 'indicacoes_admin';

  const { data: history = [], refetch: refetchHistory } = useQuery({
    queryKey: ['leadImportsHistoryReferrals'],
    queryFn: () => apiGet('/api/lead-imports?module=indicacoes'),
  ...
  const activeAgents = useMemo(() => {
    // Somente vendedores do módulo Indicações (agent_types.modules contém 'referral' ou 'all'),
    // respeitando a visibilidade do agente atual (getVisibleAgents).
    const moduleAgents = allAgents.filter(a => {
      if (a.active === false) return false;
      const type = a.agentType || a.agent_type;
      return referralAgentTypeKeys.has(type);
    });
    const list = getVisibleAgents(moduleAgents, currentAgent);
    const q = agentSearch.trim().toLowerCase();
    return q ? list.filter(a => (a.name || '').toLowerCase().includes(q)) : list;
  }, [allAgents, agentSearch, referralAgentTypeKeys, currentAgent]);

  const previewMutation = useMutation({
    mutationFn: (rows) => apiPost('/api/lead-imports/preview', { rows, module: 'indicacoes' }),
  ...
  const confirmMutation = useMutation({
    ...
    onSuccess: (data) => {
      setReport(data);
      toast.success(`${data.imported} indicações importadas com sucesso`);
  ...
  const downloadTemplate = () => {
    ...
    XLSX.writeFile(wb, 'modelo-importacao-indicacoes.xlsx');
  };
    ...
    <motion.div className="min-h-screen" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="space-y-4 md:space-y-6">
        <div className="page-header-title-section">
          <div>
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold font-display bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
              Importar Leads — Indicações
            </h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">
              Importação em massa por planilha com distribuição igualitária entre vendedores de Indicações
            </p>
          </div>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <div className="rounded-xl bg-purple-50 dark:bg-purple-900/20 p-4 text-center">
                  <p className="text-2xl font-bold text-purple-600">{report.imported}</p>
              ...
              <div>
                <h3 className="text-sm font-semibold mb-2 flex items-center gap-1.5"><Users className="w-4 h-4" />Indicações por vendedor</h3>
              ...
              <Button variant="gradient" size="sm" className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700" onClick={resetAll}>
          ...
          <>
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div
                  className="border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl p-8 text-center cursor-pointer hover:border-purple-400 transition-colors"
                ...
                <CardContent className="space-y-4">
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select value={stage} onValueChange={setStage}>
                        ...
                        <SelectContent>
                          {REFERRAL_STAGES.filter(s => !['fechado_ganho', 'fechado_perdido'].includes(s.id)).map(s => (
                  ...
                  <div>
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-64 overflow-auto p-1">
                      {activeAgents.map(a => (
                        <label
                          key={a.id}
                          className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm cursor-pointer transition-colors ${selectedAgentIds.includes(a.id) ? 'border-purple-400 bg-purple-50 dark:bg-purple-900/20' : 'border-gray-200 dark:border-gray-700'}`}
                        >
                          <Checkbox
                      ...
                      {activeAgents.length === 0 && (
                        <p className="text-sm text-gray-400 col-span-full">Nenhum vendedor do módulo Indicações disponível.</p>
                      )}
                    </div>
                  </div>
                  ...
                  {selectedAgentIds.length > 0 && (
                    <p className="text-xs text-gray-500">
                      {preview.summary.valid} indicações válidas serão divididas em rodízio entre {selectedAgentIds.length} vendedor(es):
                      aproximadamente {Math.floor(preview.summary.valid / selectedAgentIds.length)} a {Math.ceil(preview.summary.valid / selectedAgentIds.length)} indicações cada.
                  ...
                  >
                    {confirmMutation.isPending ? (
                      ...
                      <><CheckCircle2 className="w-4 h-4 mr-2" />Confirmar Importação ({preview.summary.valid} indicações)</>
                    )}
                  </Button>
                </CardContent>
              </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg">
              ...
              Histórico de Importações — Indicações
            </CardTitle>
          </CardHeader>
          <CardContent>
            {history.length === 0 ? (
                  ...
                  <tbody>
                    {history.map(h => (
                      <tr key={h.id} className="border-t border-gray-100 dark:border-gray-800">
                        ...
                        <td className="p-2 text-right text-purple-600 font-semibold">{h.importedCount}</td>
