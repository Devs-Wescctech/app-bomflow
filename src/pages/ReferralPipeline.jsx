...
function SortableReferralCard({ referral, stage, referrerData, agentData, navigate, formatCurrency, formatDate, updateReferralMutation, pendingTasksCount, TasksPopover, onStageChange, allStages }) {
  const {
    attributes,
    listeners,
    setNodeRef,
              ...
              {(() => {
                ...
                return (
                  <>
                    <button
                      disabled={!prevStage}
                      onClick={(e) => { e.stopPropagation(); e.preventDefault(); if (prevStage) onStageChange(referral.id, prevStage.id, stage.id); }}
                    ...
                    <button
                      disabled={!nextStage}
                      onClick={(e) => { e.stopPropagation(); e.preventDefault(); if (nextStage) onStageChange(referral.id, nextStage.id, stage.id); }}
                      onPointerDown={(e) => e.stopPropagation()}
                      onMouseDown={(e) => e.stopPropagation()}
                      onTouchStart={(e) => e.stopPropagation()}
                      className={`flex items-center gap-1 px-2 py-1.5 rounded-lg text-[11px] font-medium transition-all touch-target ${
