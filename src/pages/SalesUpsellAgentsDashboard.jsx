...
export default function SalesUpsellAgentsDashboard() {
  ...
  const { data: agents = [], isLoading: agentsLoading } = useQuery({
    queryKey: ['salesDashboardAgents'],
    queryFn: () => base44.entities.Agent.list(),
    enabled: canFetchData,
  });

  const { data: teams = [], isLoading: teamsLoading } = useQuery({
    queryKey: ['salesDashboardTeams'],
    queryFn: () => base44.entities.Team.list(),
    enabled: canFetchData,
  });

  const { data: leads = [], isLoading: leadsLoading } = useQuery({
    queryKey: ['salesDashboardLeads'],
    queryFn: () => upsell.entities.LeadUpsell.list('-createdDate', 5000),
    enabled: canFetchData,
  });
  ...
  const filteredLeadsByDate = useMemo(() => {
    return leads.filter(lead => {
      const leadDate = new Date(lead.createdDate || lead.created_date || lead.createdAt);
                ...
                <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    {stat.agent.photo_url ? (
                      ...
                      <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center flex-shrink-0">
