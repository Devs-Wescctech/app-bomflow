...
const selectionPagesDir = path.resolve(__dirname, '../../public/combo-multi-selection-contract');
...
export const COMBO_MULTI_SELECTION_BASE_PRODUCT_IDS = Object.freeze([299528429]);
...
export const COMBO_MULTI_SELECTION_OPTIONAL_PRODUCTS = Object.freeze({
  ...NEW_COMBO_OPTIONAL_PRODUCTS,
  thanatopraxy: Object.freeze([48337202]),
});
...
const SELECTION_OPTIONAL_TEMPLATE_FILES = Object.freeze({
  telemedicine: 'telemedicine.jpg',
  crown: 'crown.jpg',
  cremation: 'cremation.jpg',
  mileage: 'mileage.jpg',
  thanatopraxy: 'thanatopraxy.jpg',
});
...
const isEssentialDependent = (description) =>
  description.includes('ESSENCIAL') && description.includes('DEPENDENTE');

const isPetName = (description) =>
  description.includes('BOM PET') && description.includes('NOME DO PET');

const parsePet = (person, { selectionCombo = false } = {}) => {
  ...
  if (selectionCombo) {
    return {
      ...
      breed: parts[1] || '',
      color: parts[2] || '',
      size: normalized(parts[3] || person?.porte).replace(/^PORTE\s+/, ''),
...
export function buildComboMultiWellbeingContractData(
  detail = {},
  { newCombo = false, selectionCombo = false, baseProductIds = null } = {},
) {
  ...
  const baseIds = baseProductIds || (newCombo
  ...
  const dependentPeople = people
    .filter((person) => !person?.is_titular && linkedTo(person, isBomMedDependent))
  const dependents = sortComboMultiWellbeingDependents(dependentPeople.map((person, index) => ({
  ...
  const dependentValue = selectionCombo
    ? rounded(products
      .filter((product) =>
        isEssentialDependent(normalized(product?.descricao))
        && amount(product?.preco) > 1)
  ...
  const optionalProductGroups = selectionCombo
    ? COMBO_MULTI_SELECTION_OPTIONAL_PRODUCTS
    : NEW_COMBO_OPTIONAL_PRODUCTS;
  const optionalServices = (newCombo || selectionCombo)
    ? Object.fromEntries(Object.entries(optionalProductGroups).map(([key, ids]) => {
  ...
  return {
    ...
    phone2: selectionCombo
      ? ''
      : legacyComboVehicle ? text(holder.telefone) : text(detail.telefone_secundario),
    ...
    new_combo: newCombo,
    selection_combo: selectionCombo,
    optional_services: optionalServices,
    funeral_form: selectionCombo ? {
      adhesion: 0,
      standard_value: 0,
      dependent_value: standardValue,
    ...
    vehicle: vehicle ? {
      manufacturer: selectionCombo
        ? text(vehicle.fabricante)
        : legacyComboVehicle ? legacyVehicleParts[0] : text(vehicle.fabricante),
      model: selectionCombo
        ? text(vehicle.modelo)
        : legacyComboVehicle ? '' : text(vehicle.modelo),
      color: selectionCombo
        ? text(vehicle.cor)
        : legacyComboVehicle ? legacyVehicleParts[2] : text(vehicle.cor),
      year: legacyComboVehicle
        ? dateParts(vehicle.data_nascimento)?.year || ''
        : text(vehicle.ano),
      plate: (selectionCombo
        ? text(vehicle.placa).replace(/^PLACA\s+/i, '')
        : legacyComboVehicle ? legacyVehicleParts[1] : text(vehicle.placa)).toUpperCase(),
    } : null,
    driver: driver ? {
...
export function validateComboMultiWellbeingContractData(data) {
  ...
  if (data?.new_combo || data?.selection_combo) {
    const templateFiles = data.selection_combo
      ? SELECTION_OPTIONAL_TEMPLATE_FILES
      : NEW_COMBO_OPTIONAL_TEMPLATE_FILES;
    const templateDir = data.selection_combo ? selectionPagesDir : newPagesDir;
    for (const [key, service] of Object.entries(data.optional_services || {})) {
      const fileName = templateFiles[key];
      if (service?.present && !fs.existsSync(path.join(templateDir, fileName))) {
...
async function renderComboMultiWellbeingPdfFrom(data, contractPagesDir) {
  const pageCount = data.selection_combo ? 16 : (data.pet ? 17 : 16);
  ...
  return new Promise((resolve, reject) => {
    ...
    const writeForm = (pageNumber) => {
      const form = pageNumber === 9 && data.funeral_form ? data.funeral_form : data;
      write(money(form.adhesion), 29, 39.5, { size: 11 });
      write(money(form.standard_value), 52, 39.5, { size: 11 });
      write(money(form.dependent_value), 75, 39.5, { size: 11 });
      write(money(form.monthly_value), 100, 39.5, { size: 11 });
      ...
      if (data.pet) {
        ...
        const sizeY = data.selection_combo
          ? (pageNumber === 9 ? 141 : 155)
          : 142 + protocolPetOffset;
        if (sizeX) write('X', sizeX, sizeY, { size: 9 });
      }
    ...
    try {
      for (let page = 1; page <= pageCount; page += 1) {
        ...
        if (page === 8) {
          writeIssueDate(issue, data.selection_combo ? 216 : 209.75, {
            x: 132, shortYear: true, monthOffset: 18, yearOffset: 59,
          });
        }
        if (page === 16) {
          writeIssueDate(issue, data.selection_combo ? 210.5 : 216.75, {
            x: 132, shortYear: true, monthOffset: 18, yearOffset: 59,
          });
        }
        if (page === 17) {
      ...
      if (data.new_combo || data.selection_combo) {
        const optionalDir = data.selection_combo ? selectionPagesDir : newPagesDir;
        const optionalFiles = data.selection_combo
          ? SELECTION_OPTIONAL_TEMPLATE_FILES
          : NEW_COMBO_OPTIONAL_TEMPLATE_FILES;
        const addConfiguredOptionalPage = (key, renderFields) => {
          const service = data.optional_services?.[key];
          if (!service?.present) return;
          const fileName = optionalFiles[key];
          const source = path.join(optionalDir, fileName);
          if (!fs.existsSync(source)) {
            throw new Error(`Modelo do serviço opcional ausente: ${fileName}.`);
          }
          doc.addPage();
          doc.image(source, 0, 0, { width: 595.28, height: 841.89 });
          doc.fillColor('#111');
        ...
        if (data.selection_combo && data.pet) {
          const source = path.join(optionalDir, optionalFiles.telemedicine);
          doc.addPage();
          doc.image(source, 0, 0, { width: 595.28, height: 841.89 });
          doc.fillColor('#111');
          writeIssueDate(issue, 243.75, {
        ...
        addConfiguredOptionalPage('cremation', (service) => {
        ...
        addConfiguredOptionalPage('thanatopraxy', (service) => {
          write(legacyOptionalValue(service.value), 98, 158.5, { size: 11 });
          write('REAIS', 130, 158.5, { size: 11 });
          write(issue.day, 125, 241, { size: 12 });
          write(issue.month, 143, 241, { size: 12, width: 28 });
          write(issue.year.slice(-2), 182, 241, { size: 12 });
        });
        addConfiguredOptionalPage('mileage', (service) => {
        ...
        addConfiguredOptionalPage('crown', (service) => {
...
export const renderComboMultiSelectionPdf = (data) =>
  renderComboMultiWellbeingPdfFrom(data, selectionPagesDir);