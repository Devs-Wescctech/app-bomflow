...
export async function getOrcamentoDetalhe(pedidoId) {
  ...
  const pessoasRes = await db.query(
     `SELECT pp.id            AS pessoa_row_id,
            ...
            pp.pessoa_id,
            ppp.id AS link_id,
             ppp.aprovado AS link_approved,
            ppp.item_pedido_id,
            ppp.sequencia AS link_sequence,
            ipx.sequencia AS item_sequence,
             ipx.produto_id AS linked_product_id,
             ipx.situacao AS linked_item_status,
  ...
  const produtos = itensRes.rows.map((r) => ({
    ...
    sequence: r.sequencia != null ? Number(r.sequencia) : null,
    status: r.item_status || null,
  ...
  // O endereço vinculado ao pedido é o retrato do endereço contratado na data da venda.
  // O residencial ativo da Pessoa é apenas fallback para pedidos antigos sem endereco_id.
  ...
  for (const row of pessoasRes.rows) {
    ...
    if (row.link_id != null) {
      entry.product_links.push({
        link_id: Number(row.link_id),
        item_id: row.item_pedido_id != null ? Number(row.item_pedido_id) : null,
        product_id: row.linked_product_id != null ? Number(row.linked_product_id) : null,
        description: desc || null,
        item_status: row.linked_item_status || null,
        approved: row.link_approved || null,
      });
    }
  }
