...
-- Auditoria LGPD da impressão de contratos Bom Auto. CPF nunca é armazenado
-- em claro: somente HMAC/hash e os dois últimos dígitos mascarados.
CREATE TABLE IF NOT EXISTS bom_auto_contract_audit (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    action VARCHAR(20) NOT NULL CHECK (action IN ('lookup', 'generation', 'whatsapp')),
    user_id UUID,
...
ALTER TABLE bom_auto_contract_audit ADD COLUMN IF NOT EXISTS recipient_hash CHAR(64);
ALTER TABLE bom_auto_contract_audit DROP CONSTRAINT IF EXISTS bom_auto_contract_audit_action_check;
ALTER TABLE bom_auto_contract_audit
  ADD CONSTRAINT bom_auto_contract_audit_action_check
  CHECK (action IN ('lookup', 'generation', 'whatsapp'));
CREATE INDEX IF NOT EXISTS idx_bom_auto_contract_audit_created
  ON bom_auto_contract_audit(created_at DESC);

CREATE TABLE IF NOT EXISTS bom_auto_contract_whatsapp_sends (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    generation_jti UUID NOT NULL,
    user_id UUID,
    ...
    UNIQUE (generation_jti, recipient_hash)
);
ALTER TABLE bom_auto_contract_whatsapp_sends
  ADD COLUMN IF NOT EXISTS mirror_status VARCHAR(20);
ALTER TABLE bom_auto_contract_whatsapp_sends
  ADD COLUMN IF NOT EXISTS mirror_payload JSONB;
CREATE UNIQUE INDEX IF NOT EXISTS uq_bom_auto_contract_whatsapp_generation
  ON bom_auto_contract_whatsapp_sends(generation_jti);
CREATE INDEX IF NOT EXISTS idx_bom_auto_contract_whatsapp_cleanup
  ON bom_auto_contract_whatsapp_sends(object_expires_at)
...
ALTER TABLE leads_upsell ADD COLUMN IF NOT EXISTS reopened_at TIMESTAMP;
ALTER TABLE leads_upsell ADD COLUMN IF NOT EXISTS reopened_by VARCHAR(255);
...
CREATE TABLE IF NOT EXISTS referral_reactivations (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  cpf VARCHAR(11),
...
ALTER TABLE referral_reactivations ALTER COLUMN cpf DROP NOT NULL;
...
ALTER TABLE agents ADD COLUMN IF NOT EXISTS canal_venda VARCHAR(255);
ALTER TABLE agents ADD COLUMN IF NOT EXISTS canal_venda_id BIGINT;
