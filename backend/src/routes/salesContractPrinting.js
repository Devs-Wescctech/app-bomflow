...
export const CONTRACT_WHATSAPP_TEMPLATES = Object.freeze({
  ...
  [CONTRACT_PRODUCTS.PEROLA]: Object.freeze({
    id: '69ed0d552e1d23a0987f433d',
    name: 'bom_vindas_funeral',
  }),
  [CONTRACT_PRODUCTS.RUBI]: Object.freeze({
    id: '69ed0d552e1d23a0987f433d',
    name: 'bom_vindas_funeral',
  }),
  [CONTRACT_PRODUCTS.SAFIRA]: Object.freeze({
    id: '69ed0d552e1d23a0987f433d',
    name: 'bom_vindas_funeral',
  }),
  [CONTRACT_PRODUCTS.TOPAZIO]: Object.freeze({
    id: '69ed0d552e1d23a0987f433d',
    name: 'bom_vindas_funeral',
  }),
  [CONTRACT_PRODUCTS.TOTAL_MAIS_BOM_FARMA]: Object.freeze({
  ...
  [CONTRACT_PRODUCTS.COMBO_MULTI_SELECTION]: Object.freeze({
    id: '69ed0d552e1d23a0987f42bb',
    name: 'boas_vindas_multi_bem_star',
    documentHeader: false,
  }),
...
export function buildContractWhatsAppDelivery({
  ...
  const isPerola = productKey === CONTRACT_PRODUCTS.PEROLA;
  const isRubi = productKey === CONTRACT_PRODUCTS.RUBI;
  const isSafira = productKey === CONTRACT_PRODUCTS.SAFIRA;
  const isTopazio = productKey === CONTRACT_PRODUCTS.TOPAZIO;
  const isTotalMais = productKey === CONTRACT_PRODUCTS.TOTAL_MAIS_BOM_FARMA;
  ...
  const productName = productKey === CONTRACT_PRODUCTS.BOM_FAMILIA ? 'Plano Família'
    ...
    : isPerola ? 'Plano Pérola' : isRubi ? 'Plano Rubi' : isSafira ? 'Plano Safira'
      : isTopazio ? 'Plano Topázio' : isTotalMais ? 'Total Mais e Bom Farma'
  ...
  return {
    ...
    caption: isCombo
      ...
      : (isEssential || isPerola || isRubi || isSafira || isTopazio || isTotalMais)
    ...
    SELECT p.id pedido, p.pedido numero_pedido, p.contrato_id,
      ...
      JOIN LATERAL (
        SELECT 'bom_auto'::text AS product_key
         WHERE EXISTS (
          ...
          AND NOT EXISTS (
           ...
           AND NOT EXISTS (
             SELECT 1 FROM itens_pedidos combo_ip
              WHERE combo_ip.pedido_id=p.id AND combo_ip.produto_id = ANY($20::bigint[])
           )
        UNION ALL
        SELECT 'essencial'::text
         ...
         SELECT 'perola'::text
          WHERE EXISTS (
            SELECT 1 FROM itens_pedidos ip
             WHERE ip.pedido_id=p.id
               AND ip.produto_id = ANY($15::bigint[])
               AND p.situacao = 'A'
               AND ip.situacao = 'P'
               AND EXISTS (
                 SELECT 1
                   FROM pedidos_pessoas_produtos ppp
                  WHERE ppp.pedido_id = p.id
                    AND ppp.item_pedido_id = ip.id
                    AND ppp.aprovado = 'S'
               )
          )
         UNION ALL
         SELECT 'rubi'::text
          WHERE EXISTS (
            SELECT 1 FROM itens_pedidos ip
             WHERE ip.pedido_id=p.id
               AND ip.produto_id = ANY($16::bigint[])
               AND p.situacao = 'A'
               AND ip.situacao = 'P'
               AND EXISTS (
                 SELECT 1
         ...
         SELECT 'safira'::text
          WHERE EXISTS (
            SELECT 1 FROM itens_pedidos ip
             WHERE ip.pedido_id=p.id
               AND ip.produto_id = ANY($17::bigint[])
               AND p.situacao = 'A'
               AND ip.situacao = 'P'
               AND EXISTS (
                 SELECT 1 FROM pedidos_pessoas_produtos ppp
                  WHERE ppp.pedido_id = p.id
                    AND ppp.item_pedido_id = ip.id
                    AND ppp.aprovado = 'S'
               )
          ...
          SELECT 'topazio'::text
           WHERE EXISTS (
             SELECT 1 FROM itens_pedidos ip
              WHERE ip.pedido_id=p.id
                AND ip.produto_id = ANY($18::bigint[])
                AND p.situacao = 'A'
                AND ip.situacao = 'P'
                AND EXISTS (
                  SELECT 1 FROM pedidos_pessoas_produtos ppp
          ...
          UNION ALL
          SELECT 'total_mais_bom_farma'::text
           WHERE EXISTS (
             SELECT 1 FROM itens_pedidos ip
              WHERE ip.pedido_id=p.id
                AND ip.produto_id = ANY($19::bigint[])
                AND p.situacao = 'A'
                AND ip.situacao = 'P'
                AND EXISTS (
                  SELECT 1 FROM pedidos_pessoas_produtos ppp
        ...
        SELECT 'bom_pet'::text
         ...
         UNION ALL
         SELECT 'combo_multi_selecao'::text
          WHERE EXISTS (
            SELECT 1 FROM itens_pedidos ip
             WHERE ip.pedido_id=p.id
               AND ip.produto_id = ANY($20::bigint[])
          )
         UNION ALL
        ...
        SELECT 'bom_auto'::text AS product_key
         WHERE EXISTS (
          ...
          AND NOT EXISTS (
           ...
           AND NOT EXISTS (
             SELECT 1 FROM itens_pedidos combo_ip
              WHERE combo_ip.pedido_id=p.id AND combo_ip.produto_id = ANY($19::bigint[])
           )
        UNION ALL
        SELECT 'essencial'::text
         WHERE EXISTS (
        ...
        SELECT 'bom_pet'::text
         ...
         SELECT 'bom_ideal'::text
          WHERE EXISTS (
            SELECT 1 FROM itens_pedidos ip LEFT JOIN produtos pr ON pr.id=ip.produto_id
             WHERE ip.pedido_id=p.id
               AND (ip.produto_id = ANY($8::bigint[])
                 OR UPPER(COALESCE(pr.descricao, ip.descricao, '')) LIKE '%BOM DESCANSO IDEAL%BASE%')
          )
         UNION ALL
         SELECT 'bom_med'::text
...
export async function loadBomIdealContractData(claims) {
  const db = getErpPool();
...
export async function loadBomFamiliaContractData(claims) {
  const db = getErpPool();
  const result = await db.query(
    `SELECT p.pedido::text AS numero_pedido, dp.documento
       FROM pedidos p
...
const buildProductContractData = (detail, productKey) => {
  ...
  if (productKey === CONTRACT_PRODUCTS.PEROLA) return buildPerolaContractData(detail);
  if (productKey === CONTRACT_PRODUCTS.RUBI) return buildRubiContractData(detail);
  if (productKey === CONTRACT_PRODUCTS.SAFIRA) return buildSafiraContractData(detail);
  if (productKey === CONTRACT_PRODUCTS.TOPAZIO) return buildTopazioContractData(detail);
  if (productKey === CONTRACT_PRODUCTS.TOTAL_MAIS_BOM_FARMA) {
    return buildTotalMaisContractData(detail);
  }
...
const validateProductContractData = (data, productKey) => {
  ...
  if (productKey === CONTRACT_PRODUCTS.PEROLA) return validatePerolaContractData(data);
  if (productKey === CONTRACT_PRODUCTS.RUBI) return validateRubiContractData(data);
  if (productKey === CONTRACT_PRODUCTS.SAFIRA) return validateSafiraContractData(data);
  if (productKey === CONTRACT_PRODUCTS.TOPAZIO) return validateTopazioContractData(data);
  if (productKey === CONTRACT_PRODUCTS.TOTAL_MAIS_BOM_FARMA) {
    return validateTotalMaisContractData(data);
  }
...
const renderProductContract = (
  ...
  if (productKey === CONTRACT_PRODUCTS.COMBO_MULTI_SELECTION) {
    return renderComboMultiSelectionPdf(data);
  }
  if (productKey === CONTRACT_PRODUCTS.CONVALESCENCA) {
    return renderConvalescencaPdf(data);
  }
  ...
  if (productKey === CONTRACT_PRODUCTS.PEROLA) return renderPerolaPdf(data);
  if (productKey === CONTRACT_PRODUCTS.RUBI) return renderRubiPdf(data);
  if (productKey === CONTRACT_PRODUCTS.SAFIRA) return renderSafiraPdf(data);
  if (productKey === CONTRACT_PRODUCTS.TOPAZIO) return renderTopazioPdf(data);
  if (productKey === CONTRACT_PRODUCTS.TOTAL_MAIS_BOM_FARMA) {
    return renderTotalMaisPdf(data);
  }
...
const contractFileProduct = (productKey) => ({
  ...
  [CONTRACT_PRODUCTS.PEROLA]: 'perola',
  [CONTRACT_PRODUCTS.RUBI]: 'rubi',
  [CONTRACT_PRODUCTS.SAFIRA]: 'safira',
  [CONTRACT_PRODUCTS.TOPAZIO]: 'topazio',
  [CONTRACT_PRODUCTS.TOTAL_MAIS_BOM_FARMA]: 'total_mais_bom_farma',
  ...
  [CONTRACT_PRODUCTS.COMBO_MULTI_SELECTION]: 'combo_multi_selecao',
...
router.post('/contracts/send-whatsapp', async (req, res) => {
  ...
  try {
    ...
    let errors;
    if (STANDALONE_ERP_PRODUCTS.has(productKey)) {
      data = await loadStandaloneContractData(productKey, claims);
      errors = validateStandaloneContractData(productKey, data);
    } else {
      const detail = await getOrcamentoDetalhe(Number(claims.pedido));
      const titular = detail?.titular;
      if (!detail?.titular_is_canonical
      ...
      data = buildProductContractData(detail, productKey);
      errors = validateProductContractData(data, productKey);
    }
    if (errors.length) {
